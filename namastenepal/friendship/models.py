...
AUTH_USER_MODEL = getattr(settings, "AUTH_USER_MODEL", "core.User")
...
def cache_key(cache_type, user_pk):
    """
    Build the cache key for a particular type of cached value
    """
    return CACHE_TYPES[cache_type] % user_pk


def bust_cache(cache_type, user_pk):
    """
    Bust our cache for a given cache_type, can bust multiple caches
    """
...
class FriendshipRequest(models.Model):
    ...
    message: str = models.TextField(_("Message"), blank=True)

    created: datetime = models.DateTimeField(default=timezone.now)
    rejected: datetime = models.DateTimeField(blank=True, null=True)
    viewed: datetime = models.DateTimeField(blank=True, null=True)
