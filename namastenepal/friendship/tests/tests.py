...
from namastenepal.friendship.exceptions import AlreadyExistsError, AlreadyFriendsError
from namastenepal.friendship.models import Friend, Follow, FriendshipRequest, Block
...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        """
        self.user_pw = 'test1234*'
        self.user_bob = self.create_user(
            first_name="bob", last_name="man", username='bobman', email='bob@bob.com', gender='male', password=self.user_pw)
        self.user_steve = self.create_user(
            first_name="steve", last_name="man", username='steve', email='steve@steve.com', gender='male', password=self.user_pw)
        self.user_susan = self.create_user(
            first_name="susan", last_name="karen", username='susan', email='susan@susan.com', gender='female', password=self.user_pw)
        self.user_amychan = self.create_user(
            first_name="amy", last_name="chan", username='amychan', email='amy@amy.com', gender='female', password=self.user_pw)
        cache.clear()
    ...
    def create_user(self, first_name, last_name, username, email, gender, password):
        gender, _ = Gender.objects.get_or_create(title=gender)
...
class FriendshipModelTests(BaseTestCase):
    ...
    def test_following(self):
        ...
        self.assertEqual(Follow.objects.followers(
            self.user_steve), [self.user_bob])
        self.assertEqual(Follow.objects.following(
            self.user_bob), [self.user_steve])

        self.assertTrue(Follow.objects.follows(self.user_bob, self.user_steve))
        self.assertFalse(Follow.objects.follows(
            self.user_steve, self.user_bob))
        ...
        self.assertTrue(Follow.objects.remove_follower(
            self.user_bob, self.user_steve))
        ...
        with self.assertRaises(ValidationError):
            Follow.objects.create(follower=self.user_bob,
                                  followee=self.user_bob)
    ...
    def test_blocking(self):
        ...
        self.assertEqual(Block.objects.is_blocked(
            self.user_bob, self.user_steve), True)
        ...
        # Remove the relationship
        self.assertTrue(Block.objects.remove_block(
            self.user_bob, self.user_steve))
...
class FriendshipViewTests(BaseTestCase):
    ...
    def test_friendship_view_friends(self):
        url = reverse('friendship_view_friends', kwargs={
                      'username': self.user_bob.username})
    ...
    def test_friendship_add_friend(self):
        url = reverse('friendship_add_friend', kwargs={
                      'to_username': self.user_amy.username})
    ...
    def test_friendship_accept(self):
        url = reverse('friendship_accept', kwargs={
                      'friendship_request_id': self.friendship_request.pk})
        ...
        with self.login(self.user_bob.username, self.user_pw):
            ...
            response = self.client.get(url)
            self.assertResponse302(response)
            redirect_url = reverse('friendship_requests_detail', kwargs={
                                   'friendship_request_id': self.friendship_request.pk})
            self.assertTrue(redirect_url in response['Location'])
            ...
            response = self.client.post(url)
            self.assertResponse302(response)
            redirect_url = reverse('friendship_view_friends', kwargs={
                                   'username': self.user_bob.username})
            self.assertTrue(redirect_url in response['Location'])
    ...
    def test_friendship_reject(self):
        url = reverse('friendship_reject', kwargs={
                      'friendship_request_id': self.friendship_request.pk})
    ...
    def test_friendship_cancel(self):
        url = reverse('friendship_cancel', kwargs={
                      'friendship_request_id': self.friendship_request.pk})
    ...
    def test_friendship_requests_detail(self):
        url = reverse('friendship_requests_detail', kwargs={
                      'friendship_request_id': self.friendship_request.pk})
    ...
    def test_follower_add(self):
        url = reverse('follower_add', kwargs={
                      'followee_username': self.user_amy.username})
        ...
        with self.login(self.user_bob.username, self.user_pw):
            ...
            response = self.client.post(url)
            self.assertResponse302(response)
            redirect_url = reverse('friendship_following', kwargs={
                                   'username': self.user_bob.username})
            self.assertTrue(redirect_url in response['Location'])
    ...
    def test_follower_remove(self):
        ...
        url = reverse('follower_remove', kwargs={
                      'followee_username': self.user_amy.username})
    ...
    def test_block_add(self):
        url = reverse('block_add', kwargs={
                      'blocked_username': self.user_amy.username})
        ...
        with self.login(self.user_bob.username, self.user_pw):
            ...
            response = self.client.post(url)
            self.assertResponse302(response)
            redirect_url = reverse('friendship_blocking', kwargs={
                                   'username': self.user_bob.username})
            self.assertTrue(redirect_url in response['Location'])
    ...
    def test_block_remove(self):
        ...
        url = reverse('block_remove', kwargs={
                      'blocked_username': self.user_amy.username})
